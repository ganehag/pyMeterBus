...
class VIF_Unit_Multiplier_Masks(Enum):
...
class VIF_Extension_FD_Mask(Enum):
	# Currency Units
...
class Telegram_Date_Masks(Enum):
...
class DateCalculator(object):
	...
	@staticmethod
	def getTimeWithSeconds( secondValue, minuteValue, hourValue ):
		return "{0}:{1}".format(
			DateCalculator.getTime(minuteValue, hourValue),
			DateCalculator.getSeconds(secondValue)
		)

	@staticmethod
	def getTime( minuteValue, hourValue ):
		return "{0}:{1}".format(
			DateCalculator.getHour(hourValue),
			DateCalculator.getMinutes(minuteValue)
		)
	
	@staticmethod
	def getDate( dayValue, monthValue, calcHundertYear ):
		return "{0}.{1}.{2}".format( 
			DateCalculator.getDay(dayValue),
			DateCalculator.getMonth(monthValue),
			DateCalculator.getYear(dayValue, monthValue, 0, False)
		)
	
	@staticmethod
	def getDateTime( minuteValue, hourValue, dayValue, monthValue, calcHundertYear):
		return "{0} {1}".format(
			DateCalculator.getDate(dayValue, monthValue, calcHundertYear),
			DateCalculator.getTime(minuteValue, hourValue)
		)
	
	@staticmethod
	def getDateTimeWithSeconds( seconds, minuteValue, hourValue, dayValue, monthValue, calcHundertYear):
		return "{0} {1}".format(
			DateCalculator.getDate(dayValue, monthValue, calcHundertYear),
			DateCalculator.getTimeWithSeconds(seconds, minuteValue, hourValue)
		)

	@staticmethod
	def getSeconds(secondValue):
		return secondValue & DateCalculator.SECOND_MASK

	@staticmethod
	def getMinutes(minuteValue):
		return minuteValue & DateCalculator.MINUTE_MASK

	@staticmethod
	def getHour(hourValue):
